...
class FileManager:
    ...
    def make_dir(self, dirname: str, current_dir: str | Path = ".") -> Path:
        ...
        path = self.resolve_path(current_dir) / name
    ...
    def read_file(self, filename: str, current_dir: str | Path = ".") -> str:
        path = self.resolve_path(current_dir) / strip_listing_prefix(filename)
    ...
    def delete(self, name: str, current_dir: str | Path = ".") -> None:
        path = self.resolve_path(current_dir) / strip_listing_prefix(name)
        if path.is_dir():
            shutil.rmtree(path)
        else:
            path.unlink()
    ...
    def copy(self, source: str, source_dir: str | Path = ".", target: str | Path | None = None) -> Path:
        src = self.resolve_path(source_dir) / strip_listing_prefix(source)
        if target is None:
    ...
    def rename(self, source: str, new_name: str, current_dir: str | Path = ".") -> Path:
        src = self.resolve_path(current_dir) / strip_listing_prefix(source)
